...
class DEQBase(nn.Module):
    def __init__(self, args):
        ...
        self.f_alpha = args.f_alpha
        # self.b_alpha = args.b_alpha
...
class DEQIndexing(DEQBase):
    ...
    def _solve_fixed_point(
        ...
        with torch.no_grad():
            z_star, trajectory, info = self.f_solver(
                    ...
                    eps=self.f_eps, stop_mode=self.f_stop_mode, indexing=indexing, **kwargs
                    )
    ...
    def forward(
        ...
        if self.training:
            _, trajectory, info = self._solve_fixed_point(deq_func, z_init, log=log, f_alpha=self.f_alpha)
            ...
            z_star, _, info = self._solve_fixed_point(deq_func, z_init, log=log, f_thres=self.eval_f_thres, f_alpha=self.f_alpha)
    ...
    # This method is essentially same as vanilla forward but returns additionally all the itermediate hidden states h
    def forward_trajectory(
        ...
        if self.training:
            ...
            # During inference, we directly solve for fixed point
            z_star, trajectory, info = self._solve_fixed_point(deq_func, z_init, log=log, f_thres=self.eval_f_thres, f_alpha=self.f_alpha)
...
class DEQSliced(DEQBase):
    ...
    def _solve_fixed_point(
        ...
        with torch.no_grad():
            z_star, trajectory, info = self.f_solver(
                    deq_func, x0=z_init, threshold=f_thres, # To reuse previous coarse fixed points
                    eps=self.f_eps, stop_mode=self.f_stop_mode, **kwargs
                    )
    ...
    def forward(
        ...
        if self.training:
            ...
            for f_thres, produce_grad in zip(self.indexing, self.produce_grad):
                z_star, _, info = self._solve_fixed_point(deq_func, z_star, f_thres=f_thres, log=log, f_alpha=self.f_alpha)
            ...
            # During inference, we directly solve for fixed point
            z_star, _, info = self._solve_fixed_point(deq_func, z_star, f_thres=self.eval_f_thres, log=log, f_alpha=self.f_alpha)
    ...
    def forward_trajectory(
            self, deq_func, z_star, 
            log=False, sradius_mode=False, writer=None, indexing=None,
            **kwargs
            ):
        if self.training:
            z_out = []
            for f_thres, produce_grad in zip(self.indexing, self.produce_grad):
                z_star, trajectory, info = self._solve_fixed_point(deq_func, z_star, f_thres=f_thres, log=log, f_alpha=self.f_alpha)
                z_out += produce_grad(self, deq_func, z_star, writer=writer)    # See lib/grad.py for implementations
                z_star = z_out[-1]                                              # Add the gradient chain to the solver.
            ...
            # During inference, we directly solve for fixed point
            z_star, trajectory, info = self._solve_fixed_point(deq_func, z_star, 
                    f_thres=self.eval_f_thres, log=log, f_alpha=self.f_alpha, indexing=indexing)
