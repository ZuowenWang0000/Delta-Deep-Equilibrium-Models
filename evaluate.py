...
@torch.no_grad()
def create_sintel_submission(model, warm_start=False, fixed_point_reuse=False, output_path='sintel_submission', args=None, **kwargs):
...
@torch.no_grad()
def create_kitti_submission(model, output_path='kitti_submission', args=None):
...
@torch.no_grad()
def validate_chairs(model, args, **kwargs):
...
@torch.no_grad()
def validate_things(model, args, **kwargs):
...
@torch.no_grad()
def validate_sintel(model, args, **kwargs):
    ...
    for dstype in ['clean', 'final']:
        ...
        for val_id in range(len(val_dataset)):
            ...
            if args.delta_analyse:
                delta_sparsity_dicts = model.update_block.get_delta_sparsity_dicts()
                delta_stat[f'val_id_{val_id}'] = delta_sparsity_dicts
            epe = torch.sum((flow - flow_gt)**2, dim=0).sqrt()
            epe_list.append(epe.view(-1).numpy())
            rho_list.append(info['sradius'].mean().item())
            iter_list.append(info['iters'])
        ...
        results['delta_stat'] = delta_stat
    return results


@torch.no_grad()
def validate_kitti(model, args, **kwargs):
