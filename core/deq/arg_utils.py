...
def add_deq_args(parser):
    ...
    parser.add_argument('--f_solver', default='anderson', type=str, choices=['anderson', 'broyden', 'naive_solver', 'km_solver'],
                        help='forward solver to use (only anderson and broyden supported now)')
    parser.add_argument('--f_alpha', type=float, default=0.5, help='forward pass solver damping factor, used in km_solver')
    ...
    parser.add_argument('--delta_analyse', action='store_true', help="Enable delta analyse mode.")
