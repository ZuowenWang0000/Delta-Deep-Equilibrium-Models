...
class DEQFlow(nn.Module):
    def __init__(self, args):
        ...
        self.all_injec = args.all_injec

        self.delta = args.delta
        self.delta_analyse = args.delta_analyse

        if self.delta_analyse:
            self.update_block.delta_analyse_iter_counter = 0
            self.update_block.encoder_sparsity_dicts=[]
    ...
    def reset_memories(self):
        # reset delta related memories
        self.update_block.delta_analyse_iter_counter = 0
        self.update_block.encoder_sparsity_dicts=[]
        self.update_block.reset_memories()
    ...
    def forward(self, image1, image2, 
        ...
        # TODO: it should be ok to merge func and delta_func. TODO2: I changed my mind. It's better to keep them separate.
        # for FPS measurement later.
        def delta_func(h, c):
            if not self.args.all_grad:
                c = c.detach()
            with autocast(enabled=self.args.mixed_precision):                   
        ...
        def delta_analyse_func(h,c):
            if not self.args.all_grad:
                c = c.detach()
            with autocast(enabled=self.args.mixed_precision):   
                if self.args.delta_analyse:                
                    new_h, delta_flow = \
                        self.update_block.forward_delta_analyse(h, inp, corr_fn(c), c-coords0, attn) # corr_fn(coords1) produces the index correlation volumes
            new_c = c + delta_flow  # F(t+1) = F(t) + \Delta(t)
            return new_h, new_c                      

        if self.delta:
            if not self.delta_analyse:
                deq_func = DEQWrapper(delta_func, (net, coords1))
            else:
                deq_func = DEQWrapper(delta_analyse_func, (net, coords1))
        else:
    ...
    # this method is essentially same as forward but returns additionally all the hidden states
    def forward_trajectory(self, image1, image2, 
            flow_gt=None, valid=None, fc_loss=None, 
            flow_init=None, cached_result=None, 
            writer=None, sradius_mode=False, 
            **kwargs):
        ...
        z_shape = deq_func.z_shape
        z_indexing = deq_func.z_indexing
            
        log = (inp.get_device() == 0 and np.random.uniform(0,1) < 2e-3)

        z_out, trajectory, info = self.deq.forward_trajectory(deq_func, z_init, log, sradius_mode, **kwargs)
        flow_pred = [self._decode(z, coords0) for z in z_out]
        ...
        if self.training:
            ...
            return coords1 - coords0, flow_up, \
                {"sradius": info['sradius'], \
                 "cached_result": (net, coords1 - coords0), \
                "trajectory": trajectory, "z_shapes": z_shape, "z_indexing": z_indexing}
