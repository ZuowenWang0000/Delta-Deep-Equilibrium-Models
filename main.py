...
class Logger:
    ...
    def write_dict(self, results):
        ...
        for key in results:
            # first check if the key is a dictionary
            if isinstance(results[key], dict):
                for subkey in results[key]:
                    self.writer.add_scalar(f"{key}/{subkey}", results[key][subkey], self.total_steps)
            else:
...
def train_once(args):
    ...
    if args.restore_ckpt_for_train is not None:
        assert args.restore_name is None, "Cannot restore from both name and ckpt"
        model.load_state_dict(torch.load(args.restore_ckpt_for_train), strict=True)
        print(f'Load from {args.restore_ckpt_for_train} for fine-tuning!')
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(train_loader):
            ...
            # print(f"image1 shape: {image1.shape}, image2 shape: {image2.shape}, flow shape: {flow.shape}, valid shape: {valid.shape}")
            if args.add_noise:
            ...
            if (total_steps + 1) % args.save_interval == 0:
                PATH = '%d_%s.pth' % (total_steps+1, args.name_per_run)
                save_dir = os.path.join(args.output_path, 'checkpoints')
            ...
            if total_steps % args.eval_interval == args.eval_interval - 1:
                ...
                for val_dataset in args.validation:
                    if val_dataset == 'chairs':
                        res = evaluate.validate_chairs(model.module, args, sradius_mode=args.sradius_mode, best=best_chairs)
                        best_chairs['epe'] = min(res['chairs'], best_chairs['epe'])
                        results.update(res)
                    elif val_dataset == 'things':
                        results.update(evaluate.validate_things(model.module, args, sradius_mode=args.sradius_mode))
                    elif val_dataset == 'sintel':
                        res = evaluate.validate_sintel(model.module, args, sradius_mode=args.sradius_mode, best=best_sintel)
                        best_sintel['clean-epe'] = min(res['clean'], best_sintel['clean-epe'])
                        best_sintel['final-epe'] = min(res['final'], best_sintel['final-epe'])
                        results.update(res)
                    elif val_dataset == 'kitti':
                        res = evaluate.validate_kitti(model.module, args, sradius_mode=args.sradius_mode, best=best_kitti)
...
def val(args):
    ...
    for val_dataset in args.validation:
        if val_dataset == 'chairs':
            evaluate.validate_chairs(model.module, sradius_mode=args.sradius_mode, args=args)
        elif val_dataset == 'things':
            evaluate.validate_things(model.module, sradius_mode=args.sradius_mode, args=args)
        elif val_dataset == 'sintel':
            results = evaluate.validate_sintel(model.module, sradius_mode=args.sradius_mode, args=args)
            # save results dict in a numpy file
            np.save(os.path.join(args.output_path, 'sintel_val_results.npy'), results)
        elif val_dataset == 'kitti':
            results = evaluate.validate_kitti(model.module, sradius_mode=args.sradius_mode, args=args)
            # save results dict in a numpy file
            np.save(os.path.join(args.output_path, 'kitti_val_results.npy'), results)
    ...
    for test_dataset in args.test_set:
        if test_dataset == 'sintel':
            evaluate.create_sintel_submission(model.module, output_path=args.output_path,
                    fixed_point_reuse=args.fixed_point_reuse, warm_start=args.warm_start, args=args)
        elif test_dataset == 'kitti':
            evaluate.create_kitti_submission(model.module, output_path=args.output_path, args=args)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--delta_threshold', type=float, default=0.0, help="Threshold for delta mode.")
    # parser.add_argument('--delta_analyse', action='store_true', help="Analyse the delta in DEQ.")
    ...
    parser.add_argument('--restore_ckpt_for_train', help="restore checkpoint for fine-tuning")
    ...
    parser.add_argument('--all_injec', default=False, action="store_true", help="If used, inject data into all layers of DEQ.")
